...
function MovieDetail() {
  ...
  useEffect(() => {
    ...
    const checkCollectionStatus = async () => {
      try {
        ...
        const movieExists = collection.some((item) => item.movie_details?.tmdb_id === parseInt(tmdbId));
        setIsInCollection(movieExists);
        const userMovie = collection.find((item) => item.movie_details?.tmdb_id === parseInt(tmdbId));
  ...
  const getImageUrl = (path, size = "w500") =>
    path ? `https://image.tmdb.org/t/p/${size}${path}` : "https://placehold.co/300x450?text=No+Image";
  ...
  return (
    <div className="movie-detail-container" style={{ backgroundImage: `url(${backdropUrl})` }}>
      {showAlert && (
      ...
      <main>
        <div className="movie-header">
          ...
          {isInCollection ? (
            <button
              className={`remove-from-collection ${isRemoving ? "animate-remove" : ""}`}
                ...
                <p>
                  ...
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      size={16}
                      className={`star-icon ${i < userRating ? "solid" : ""}`}
                    />
                  ))}
                </p>
              ) : (
        ...
        <div className="movie-main-content">
          <div className="movie-poster-section">
            ...
            <div className="movie-stats">
              ...
              <div className="imdb-rating">
                <Star className="star-icon" />
                <span>{movie.rotten_tomatoes_rating ? `${movie.rotten_tomatoes_rating}%` : "N/A"} RT</span>
              </div>
            </div>
          </div>
          ...
          {recommendedMovies.length === 0 ? (
            ...
            <div className="recommended-grid">
              {recommendedMovies.map((recMovie) => (
                ...
                >
                  <img src={getImageUrl(recMovie.poster_path)} alt={recMovie.title} />
